...
type Post={id:number|string;slug:string;title:string;excerpt:string|null;cover_url:string|null;publish_date:string|null;};
export default function BlogIndex(){
  const [posts,setPosts]=useState<Post[]>([]),[loading,setLoading]=useState(true);
  useEffect(()=>{(async()=>{
    const {data}=await supabase.from("posts")
      .select("id,slug,title,excerpt,cover_url,publish_date")
      .eq("published",true).order("publish_date",{ascending:false}).limit(100);
  ...
  return <div className="mx-auto max-w-6xl px-6 py-12">
    ...
    {loading? <div>Loading…</div> :
      <div className="blog-grid">
        {posts.map(p=><article key={String(p.id)} className="blog-card">
